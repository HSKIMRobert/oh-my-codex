...
export type AdvisoryFenceState = 'pending_closeout' | 'recovery_required' | 'closed' | 'abandoned';
...
function fenceValid(value: Record<string, unknown>, activation: AdvisoryActivation): boolean {
  const states: AdvisoryFenceState[] = ['pending_closeout', 'recovery_required', 'closed', 'abandoned'];
...
async function readProjectionForGeneration(cwdInput: string, sessionId: string, generationId: string): Promise<AdvisoryProjection> {
  ...
  for (let index = 1; ; index += 1) {
    ...
    if (!event) break;
    // Historical/forged local release records are inert compatibility residue.
    // They never change the durable terminal planning projection or authorize
    // host execution.
    if (event.state === 'released') break;
  ...
  const priorFenceBytes = adminRaw && terminalFence
    ? await readFile(join(dir, terminalFence.sequence === 1 ? 'fence.json' : `fence-event-${String(terminalFence.sequence - 1).padStart(4, '0')}.json`)).catch(() => null)
    : null;
  ...
  if (adminRaw && (adminRaw.schema_version !== 1 || adminRaw.action !== 'abandon'
    ...
    || terminalFence?.state !== 'abandoned' || !priorFenceBytes || sha256(priorFenceBytes) !== adminRaw.prior_fence_sha256
...
export async function readCurrentRalplanAdvisory(cwdInput: string, sessionId: string): Promise<AdvisoryProjection | null> {
  ...
  // Terminal Advisory history is auditable but does not own or authorize the
  // later host workflow. A subsequent standard Ralplan may replace the live
  // mode binding without rewriting the completed Advisory generation.
  if (projection.fence && ['closed', 'abandoned', 'recovery_required'].includes(projection.fence.state)) return projection;
